...
def extract_template_data(body, issue_number=None, issue_class='issue', SECTIONS=SECTIONS):
    ...
    logging.debug('uppercase the body: {}'.format(type(body)))
    try:
    ...
    logging.debug('map out the section locations')
    match_map = {}
    ...
    if not match_map:
        logging.debug('no section matches')
        return {}

    # what are the header(s) being used?
    logging.debug('map out the headers')
    headers = []
    ...
    logging.debug('research with common header')
    ...
    logging.debug('insert issue type header')
    ...
    logging.debug('extract each section')
    ...
    logging.debug('cleanup sections')
    for k,v in tdict.items():
        ...
        try:
            ...
            try:
                ...
                logging.error('failed to remove markdown')

            # remove non-ascii chars
            #try:
            #    v = v.encode('ascii',errors='ignore')
            #except Exception as e:
            #    logging.error('failed to encode')

            # normalize newlines and return chars
            try:
                v = v.replace('\r', '\n')
            except Exception as e:
                logging.error('could not replace newlines: {}'.format(e))

            # remove pre-ceding and trailing newlines
            try:
                v = v.strip()
            except Exception as e:
                logging.error('could not strip')
    ...
    logging.debug('return the tdict')
    return tdict
